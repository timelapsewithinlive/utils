...
public class AhoCorasickDoubleArrayTrie<V> implements Serializable {
        ...
        for (int i = 0; i < text.length(); ++i) {
            ...
            if (hitArray != null) {
                for (int hit : hitArray) {
                    processor.hit(position - keyLength[hit], position, value[hit]);
                }
            }
            ++position;
        }
    ...
    public void parseText(String text, IHitCancellable<V> processor) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (hitArray != null) {
                for (int hit : hitArray) {
                    boolean proceed = processor.hit(position - keyLength[hit], position, value[hit]);
                    if (!proceed) {
                        return;
                    }
                }
    ...
    public void parseText(char[] text, IHitFull<V> processor) {
        ...
        for (char c : text) {
            ...
            if (hitArray != null) {
                for (int hit : hitArray) {
                    processor.hit(position - keyLength[hit], position, value[hit], hit);
                }
            }
            ++position;
        }
    ...
    public Hit<V> findFirst(String text) {
        ...
        for (int i = 0; i < text.length(); ++i) {
            ...
            if (hitArray != null) {
                ...
                return new Hit<V>(position - keyLength[hitIndex], position, value[hitIndex]);
            }
            ++position;
        }
        return null;
    ...
    private void storeEmits(int position, int currentState, List<Hit<V>> collectedEmits) {
        ...
        if (hitArray != null) {
            for (int hit : hitArray) {
                collectedEmits.add(new Hit<V>(position - keyLength[hit], position, value[hit]));
            }
        }
    }
    ...
    private class Builder {
        ...
        public void build(Map<String, V> map) {
            ...
            keyLength = new int[value.length];
        ...
        private void addKeyword(String keyword, int index) {
            ...
            keyLength[index] = keyword.length();
        }
