...
public class SwrdDynamicProxy {
     ...
     * 生成并返回一个代理对象
     *
     * @param swrdClassLoader       自实现的类加载器
     * @param interfaces             被代理类所实现的所有接口
     * @param swrdInvocationHandler 一个{@link SwrdInvocationHandlerImpl}接口的实现
     *                               我们代理类对其代理的对象增强的代码写在对该接口的实现中
     *                               {@link SwrdInvocationHandlerImpl#invoke(Object, Method, Object[])}
     * @return 返回生成的代理对象
     */
    public static Object newProxyInstance(SwrdClassLoader swrdClassLoader,
                                          Class<?>[] interfaces,
                                          SwrdInvocationHandler swrdInvocationHandler) {
        try {
            ...
            Class proxyClass = swrdClassLoader.findClass(SRC_NAME);
            Constructor constructor = proxyClass.getConstructor(SwrdInvocationHandler.class);

            // 4.返回动态生成的代理对象
            return constructor.newInstance(swrdInvocationHandler);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    private static File generateSrcToFile(Class<?>[] interfaces) {
        try {
            ...
            // 将生成的字节码写入到磁盘文件
            String path = SwrdDynamicProxy.class.getResource("").getPath();
